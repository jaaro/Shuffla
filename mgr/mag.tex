...
\pawel{Zaczynasz od definicja, ale właściwie nie wiadomo po co ta definicja. Może najpierw napisz, że chcesz mieć wiele tabel, na których będziesz wykonywał inserty/search, a pojedyncza tabela to...}
...
There are different possible conditions for different types. Possible conditions for numbers are comparisons, inequalities and strict inequalities \pawel{jak dla mnie to compare to sprawdzenie czy coś jest mniejsze/większe}. Strings are compared using lexicographic order so comparisons, inequalities and strict inequalities should be supported. There are two additional conditions
\begin{enumerate}
...
\end{enumerate}
Presenting results:\pawel{a może napisz to pełnym zdaniem, użytkownik ma możliwość wyboru kolejności blablabla}
\begin{enumerate}
...
\end{enumerate}
\pawel{to jest troszeczkę naciągane moim zdaniem}
...
\pawel{napisz co to jest relational database}
...
\pawel{Może zacznij ten kawałek od stwierdzenia, że Sphinx ma dużo minusów, i teraz je wymienisz?}
...
\bigskip
\pawel{ten kawałek kończy section? czy subsection? może powinien być jakoś wydzielony, lub znajdować się na początku section?}
...
\pawel{te half-planes to są takie specjalne, nie? może lepiej byłoby napisać orthogonal range query...}
...
Complexities (where $d \geq2$) \pawel{mógłbyś napisać tutaj pełne zdanie? I zdefiniować $d$?}
...
\pawel{Ostatecznie tak nie robisz, więc... hm}
...
\pawel{w zasadzie nie napisałeś jaki wygląda algorytm. Więc może fajnie byłoby zdefiniować K-D tree i napisać jak są wykonywane poszczególne operacje? Oraz narysować przykład, tj zbiór punktów, który masz na późniejszych obrazkach, i odpowiadające mu drzewko?}

\section{Implementation}

\subsection{HTTP Service}

\pawel{Odnośniki do rysunków powinny być choć minimalnie zintegrowane z tekstem}
...
I decided that it would be best if search engine could mimic RESTful service \pawel{a mógłbyś tutaj wyjaśnić co to znaczy?}. Every search/insert/delete request should be sent as a HTTP request. Server works in the multi-threaded environment \pawel{trochę się pogubiłem, każdy request jest w nowym wątku? czy jak?}. As a base I used \verb|example2| \cite{ASIOHTTP} from \verb|boost::asio| documentation which fulfills my requirements. Threads are managed by \verb|boost::io_service| \cite{IOSERVICE}. There is another thread for periodical functions. Currently its invoking only saving snapshots.
...
\item measuring running time (slow log purpose \pawel{napisz co to jest slow log}),
...
\pawel{mógłbyś napisać to pełnym zdaniem? I zdefiniować co to znaczy locking?}
...
\pawel{dopisz tu jeszcze jakieś uzasadnienie, że taka implementacja jest ok}

\subsection{How data is stored}

\pawel{znowu: mógłbyś zacząć sekcję od pełnego zdania?}
Requirements:
\begin{enumerate}
\item all data can be easily serializable,
\item it should somehow force type safety, \pawel{jakoś mi to somehow nie leży, zresztą, faktycznie zapewniasz to type safety?}
...
\item for each column I store pointer to \verb|Type| which contains value of this column. \pawel{row stores, ale potem for each column I store? to kto, kogo, i za co w końcu trzyma? ;)}
\end{enumerate}

\pawel{TypeString to char*? czy string?}

\subsection{Database persistence}

\pawel{zdefiniuj persistence}
...
\pawel{tutaj widziałbym jakiś akapicik, w którym mówisz, że o ostatecznej efektywności decydują szczegóły, a niżej opiszesz kilka z nich}
...
K-D tree has similar problem to binary trees. Binary tree can be unbalanced and so can K-D tree. My idea for balancing K-D tree is very simple. When number of nodes in one subtree is 5 times larger that number of nodes in opposite subtree then I am rebuilding their parent subtree from the beginning. \pawel{5 times czy 2 times? te późniejsze rachunki zakładają 2...}
This is known as partial rebuilding, a simple yet very powerful paradigm often used for making data structures dynamic, for example in weight-balanced trees~\cite{ALPHATREES}. While the worst case complexity of a single operations can be large because of this rebalancing, the amortized complexity~\cite{AMOR} is actually rather good. \pawel{zdefiniuj amortized complexity}


\section{Complexities}

\pawel{vertex to zwykle $v$}
...
\pawel{ja bym ten warunek napisał zdaniem, zamiast >= i <= powinny być specjalne znaczki}
...
\pawel{na początku powinno być jakieś zdanie wstępu}
...
\pawel{to chyba powinno być na samym końcu...}
