...
\end{enumerate}
For example if query is first name = John $\wedge$  LIMIT 1 then we are only interested in word John from inverted index. Engine finds first occurence and returns it. If query is first name = John $\wedge$ SUBSTRING(last name) = qwertyabc LIMIT 1 then it is likely that there are more Johns then all people with last name containing qwertyabc. Engine could use suffix tree which contains all words from inverted index. It would find that there is nobody with such last name. Such output would be rendered very fast, probably even without reading inverted index. In worst case scenario algorithm works in linear time. Example for such a case could be SUBSTRING(last name)=a $\wedge$ SUBSTRING(last name) = b $\wedge$ ... $\wedge$ SUBSTRING(last name) = z LIMIT 1. Almost whole inverted index must be processed but still result set will be empty.
...
\bigskip
It seems that none of existing solutions are processing search queries very fast. In worst case scenario it's always $\Oh(n)$. All solutions are working on data stored on disk. In age when RAM is cheap and servers with +32GB RAM is \todo{english for chleb powszedni}. Although data stored on disk could be transfer to RAM by ramdisks, it shows that such solution isn't primary target to software creators. For me it seems that in this area is room for improvement. That's why I decided to create my own search engine.  
...
\jarek{Tu fajnie byłoby wrzucić diagram i na podstawie niego opisywać co się dzieje}
I decided that it would be best if search engine could mimic RESTful service. Every search/insert/delete request should be sent as a HTTP request. Server works in the multi-threaded environment. As a base I used $example3$ \cite{ASIOHTTP} from \verb|boost::asio| documentation which fulfills my requirements. \todo{Jak działa example3, pula wątków}

Every request is converted to a corresponding class. For example, search request is converted to SearchQuery, insert is converted InsertQuery, and so on. At this level, syntax validation is performed. In case of an error, HTTP error code 400 is returned, and error message is appended to the log file with errors.
...
Let $n$ be number of points in the tree. Tree with root $\theta$ is well-balanced if and only if  sub so it's height is $\Oh(\log n)$. Every point belongs to $\Oh(\log n)$ nodes. 

Assumptions:
\begin{enumerate}
\item all values of all columns are different
\item rebuilding subtree with root $\theta$ causes $\theta$ to be well-balanced.
\end{enumerate}
...
\todo{Schema create table}

\subsection{Sphinx search}

For this problem real-time indexes are best \cite{SPHINXRT}. Scheme is the same as for PostgreSQL.
