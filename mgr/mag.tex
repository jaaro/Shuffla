...
To simplify the analysis, the assume that the values in all columns are unique. We will ensure that the tree is always well-balanced by rebuilding its subtrees that violate this property. We assume that whenever we rebuild a subtree, it becomes super-balanced. While it is possible to actually ensure that it becomes perfectly balanced, for the reasons explained in Section~\ref{chapter:processing} before the actual implementation guarantees a weaker 
